...
{
echo "<h1>Editer une &eacute;quipe</h1>";
include("connection.php");
$sql = "SELECT * FROM `equipes` WHERE `id`='".$_POST['id']."'";
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
...
$trainer = $data['entraineur'];
$ent1 = $data['entrainement1'];
$ent2 = $data['entrainement2'];
$url = $data['url_classement'];
$photo = $data['url_photo'];
$afficher = $data['afficher'];
mysql_close();
?>
<form method="post" action="equipes.php">
<p><input type="hidden" name="id" value="<? echo $id; ?>"><input type="hidden" name="oldteam" value="<? echo $division; ?>"></p>
<p>Attention : une modification de la valeur 'division' aura une influence sur les membres et les matches associ&eacute;s</p>
<p>Division : <input type="text"  name="division" maxlength=4 value="<? echo $division; ?>"></p>
<p>Niveau dans le club (DH -> P4, ladies first, on démarre à 1) : <input type="text"  name="importance" maxlength=2 value="<? echo $importance; ?>"></p>
<p>Nom complet : <input type="text"  name="complet" maxlength=30 value="<? echo $nom_complet; ?>"></p>
<p>Entraineur : <input type="text"  name="entraineur" maxlength=50 value="<? echo $trainer; ?>"></p>
<p>Premier entrainement (jour heure_d&eacute;but - heure_fin) : <input type="text"  name="entrainement1" maxlength=25 value="<? echo $ent1; ?>"></p>
<p>Deuxi&egrave;me entrainement : <input type="text"  name="entrainement2" maxlength=25 value="<? echo $ent2; ?>"></p>
<p>Lien vers les classements : <input type="text"  name="url_classement" maxlength=100 value="<? echo $url; ?>"></p>
<p>URL de la photo d'&eacute;quipe : <input type="text"  name="url_photo" maxlength=200 value="<? echo $photo; ?>"></p>
<p>Afficher sur la page &eacute;quipes?: <input type="checkbox"  name="afficher" <?php echo ($afficher==1)?" checked":"";?>></p>
<p><input type="submit" name="editTeamConfirm" value="OK" />
</p>
</form>
<?
}

if(isset($_POST['addPlayerToTeam']))
{
include("connection.php");
echo '<h1>Ajouter un joueur en '.$_GET['eq'].'</h1>';
echo '<form method="post" action="equipes.php?eq='.$_GET['eq'].'">';
?>
<p><select name="id">
<?
$sql = "SELECT * FROM `membres` ORDER by equipe";
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
while($data = mysql_fetch_assoc($req)) {	
	echo '<option value="'.$data['id'].'">'.$data['prenom'].' '.$data['nom'].' ('.$data['equipe'].')</option>';
}
mysql_close();
?>
</select>
<p><input type="submit" name="addPlayerToTeamConfirm" value="OK" />
</p>
</form>
<?
}
...
$sqlb = "UPDATE phpbb3_users SET `user_access`='".$_POST['user_access']."' WHERE `user_id`='".$_POST['user_id']."'";
...
if(isset($_POST['editTeamConfirm'])){
include("connection.php");
if($_POST['afficher']){$afficher=1;} else {$afficher=0;}
...
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
while($data = mysql_fetch_assoc($req)) {
	        echo '<option value="'.$data['user_id'].'">'.$data['username'].' ('.$data['user_access'].')</option>';
}
mysql_close();
?>
</select>
<select name="user_access"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option>
</select>
...
$data = mysql_fetch_assoc($req);
	...
	if($data['entrainement1']!='NC') {
		...
		if($data['entrainement2']!='NC') echo '<li>'.$data['entrainement2'].'</li></ul>';
	}
	else { echo '</ul></td><td>'; }
	...
	echo '<p>';
	echo '<form style="display:inline;" action="equipes.php" method="post">Editer l\'&eacute;quipe : <input type="hidden" name="id" value="'.$data['id'].'">';
	echo '<input type="image" src="images/b_edit.png" value="Editer team" name="editteam"></form></p><p>';
	echo '<form style="display:inline;" action="equipes.php?eq='.$eq.'" method="post">Ajouter un joueur existant &agrave; l\'&eacute;quipe : <input type="hidden" name="id" value="'.$data['id'].'">';
	echo '<input type="image" src="images/icons/add-user.png" value="Ajouter joueur" name="addPlayerToTeam"></form></p>';
	}
